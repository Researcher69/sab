--[[
    AUTOJOINER LUA - ANDROID/MUMU VERSION
    Modified to work with Android devices and MuMu emulator
    
    For MuMu Emulator:
    - Change WebSocketURL to your Windows host IP
    - Example: "ws://192.168.1.100:51948"
    
    For Android with Termux:
    - Use "ws://127.0.0.1:51948" (localhost)
    
    For Scanner Only (No Auto-Join):
    - Use BrainrotFinderWebhook.lua instead
--]]
...
(function()
    ...
    -- For MuMu: Use your Windows host IP (e.g., "ws://192.168.1.100:51948")
    -- For Termux/Android: Use "ws://127.0.0.1:51948"
    -- To find Windows IP: Open PowerShell → ipconfig → Look for "IPv4 Address"
    local WebSocketURL = "ws://127.0.0.1:51948"  -- CHANGE THIS TO YOUR WINDOWS IP FOR MUMU!
    ...
    local function connect()
        prints("Android/MuMu AutoJoiner Starting...")
        ...
        while true do
            ...
            if success and socket then
                prints("✅ Connected to WebSocket server!")
                local ws = socket
                ...
                prints("❌ Unable to connect to websocket")
                prints("   Check if Python autojoiner is running on Windows host")
                prints("   For MuMu: Make sure WebSocketURL uses Windows IP, not 127.0.0.1")
                prints("   Trying again in 3 seconds...")
                wait(3)
            end
        end
    end
...
-- Android/MuMu compatible version
